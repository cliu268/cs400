// B. Cow Frisbee
// https://www.xinyoudui.com/contest?courses=453&books=473&pages=13018&fragments=32711&problemId=15525
/*
Farmer John's N cows (N≤3×10^5) have heights 1,2,…,N. One day, the cows are standing in a line in some order playing frisbee; 
let h1…hN denote the heights of the cows in this order (so the h's are a permutation of 1…N).
...
Output the sum of distances of all pairs of locations at which there are cows that can throw the frisbee back and forth. Note that the 
large size of integers involved in this problem may require the use of 64-bit integer data types (e.g., a "long long" in C/C++).
...
int main() {
    ...
    ll ans=0;
    for (ll i=0; i<n; i++) {
        ll curr=vect[i].first;
        ll index=vect[i].second;
        if (next[index]!=-1) {
            ans+=next[index]-index+1;
            prev[next[index]]=prev[index];
        }
        if (prev[index]!=-1) {
            ans+=abs(prev[index]-index)+1;
